...
/**
 * Creates an instance of the Mangacore Wallet Service.
 * @constructor
 */
 ...
 * @param {string} opts.network[='livenet'] - The Mangacoin network for this wallet.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Mangacore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.getWalletFromIdentifier = function(opts, cb) {
  ...
  async.parallel([
    ...
    async.detectSeries(networks, function(network, nextNetwork) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  var xPubKey;
  try {
    xPubKey = Mangacore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
 ...
 * @param {string} opts.unit - Mangacoin unit used to format amounts in notifications.
 */
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['MANGA'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Mangacore.Address(addresses[0]).toObject().network;
 ...
 * @param {string} [opts.network = 'livenet'] - The Mangacoin network to estimate fee levels from.
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var mangacoreTx = txp.getMangacoreTx();
    mangacoreError = mangacoreTx.getSerializationError(serializationOpts);
    if (!mangacoreError) {
      txp.fee = mangacoreTx.getFee();
    }
  } catch (ex) {
    self.logw('Error building Mangacore transaction', ex);
    return ex;
  }

  if (mangacoreError instanceof Mangacore.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (mangacoreError instanceof Mangacore.errors.Transaction.DustOutputs)
    return Errors.DUST_AMOUNT;
  return mangacoreError;
};
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Mangacore.Transaction.DUST_AMOUNT);
 ...
 * @param {string} [opts.network = 'livenet'] - The Mangacoin network for this transaction.
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getMangacoreTx().uncheckedSerialize();
